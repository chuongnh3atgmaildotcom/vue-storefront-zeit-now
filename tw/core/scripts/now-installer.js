...
class Backend extends Abstract {
  ...
  cloneRepository () {
    return new Promise((resolve, reject) => {
      if (shell.test('-d', this.answers.backend_dir)) {
        Message.info(`'${this.answers.backend_dir}' already exist, skipping...`)
        resolve()
        return
      }
  ...
  dockerComposeUp () {
    return new Promise((resolve, reject) => {
      ...
      // create network
      if (shell.exec(`$(which docker) network create --driver=bridge --attachable=true --label=com.docker.compose.network=default --label=com.docker.compose.project=vue-storefront-api --label=com.docker.compose.version=1.22.0 vue-storefront-api_default`).code !== 0) {
        reject(new Error('Can\'t create docker network.'))
        return
      }

      // es
      if (shell.cd('docker/elasticsearch').code !== 0) {
        reject(new Error(`Can't change directory to 'vue-storefront-api/docker/elasticsearch'.`))
        return
      }
      if (shell.exec(`docker build -t vuedeploy_elasticsearch .`).code !== 0) {
        reject(new Error('Can\'t build es image.'))
        return
      }
      if (shell.cd(`../..`).code !== 0) {
        reject(new Error('Can\'t change dir to vue-storefront-api'))
        return
      }
      if (shell.exec(`docker run -d -l "label.vuedeploy=elasticsearch" --network=vue-storefront-api_default -h elasticsearch -p 9200:9200 -p 9300:9300 -v $(pwd)/esdat1:/usr/share/elasticsearch/data --mount type=bind,source=$(pwd)/docker/elasticsearch/config/elasticsearch.yml,target=/usr/share/elasticsearch/config/elasticsearch.yml,ro=true -e ES_JAVA_OPTS="-Xmx512m -Xms512m" --name elasticsearch vuedeploy_elasticsearch`).code !== 0) {
        reject(new Error('Can\'t start es in background.'))
        return
      }
      // kibana
      if (shell.cd('docker/kibana').code !== 0) {
        reject(new Error(`Can't change directory to 'vue-storefront-api/docker/kibana'.`))
        return
      }
      if (shell.exec(`docker build -t vuedeploy_kibana .`).code !== 0) {
        reject(new Error('Can\'t build kibana image.'))
        return
      }
      if (shell.cd(`../..`).code !== 0) {
        reject(new Error('Can\'t change dir to vue-storefront-api'))
      ...
      if (shell.exec(`docker run -d  -l "label.vuedeploy=kibana" --network=vue-storefront-api_default -h kibana -p 5601:5601 --mount type=bind,source=$(pwd)/docker/kibana/config/,target=/usr/share/kibana/config/,ro=true --name vue-storefront-api_kibana_1 vuedeploy_kibana`).code !== 0) {
        reject(new Error('Can\'t start es in background.'))
        return
      }
      // redis
      if (shell.exec(`docker run -d  -l "label.vuedeploy=redis" --network=vue-storefront-api_default -h redis -p 6379:6379  --name vue-storefront-api_redis_1 redis:4-alpine`).code !== 0) {
        reject(new Error('Can\'t start redis in background.'))
        return
      }
      // app
      if (shell.exec(`docker build -f docker/vue-storefront-api/Dockerfile -t vuedeploy_api .`).code !== 0) {
        reject(new Error('Can\'t build api endpoint in background.'))
        return
      }
      if (shell.exec(`docker run -d  -l "label.vuedeploy=app"  --network=vue-storefront-api_default  -h app -p 8080:8080 --tmpfs /var/www/dist -v $(pwd)/config:/var/www/config -v $(pwd)/ecosystem.json:/var/www/ecosystem.json -v $(pwd)/migrations:/var/www/migrations -v $(pwd)/package.json:/var/www/package.json -v $(pwd)/scripts:/var/www/scripts -v $(pwd)/src:/var/www/src -v $(pwd)/var:/var/www/var --env-file docker/vue-storefront-api/default.env -e VS_ENV="dev" --name vue-storefront-api_app_1 vuedeploy_api`).code !== 0) {
        reject(new Error('Can\'t start api endpoint in background.'))
        return
      }

      // attach docker network connect --alias app vue-storefront-api_default vue-storefront-api_app_1
      // Adding 30sec timer for ES to get up and running
  ...
  createConfig () {
    return new Promise((resolve, reject) => {
      ...
      if (exists(TARGET_BACKEND_CONFIG_FILE)) {
        Message.info(`Backend config file exist:'${TARGET_BACKEND_CONFIG_FILE}', skipping...`)
        resolve()
        return
      }
      try {
  ...
  migrateElasticSearch () {
    return new Promise((resolve, reject) => {
      // additional step to rebuild index
      if (shell.exec(`npm run db rebuild -- --indexName=vue_storefront_catalog >> ${Abstract.infoLogStream}`).code !== 0) {
        reject(new Error('Can\'t rebuild.'))
      }
      
      Message.info('Migrating data into ElasticSearch...')

      if (shell.exec(`npm run migrate >> ${Abstract.infoLogStream}`).code !== 0) {
  ...
  cloneMagentoSampleData () {
    return new Promise((resolve, reject) => {
      if (shell.test('-d', SAMPLE_DATA_PATH)) {
        Message.info(`'${SAMPLE_DATA_PATH}' already exist, skipping...`)
        resolve()
        return
      }
...
class Storefront extends Abstract {
  ...
  createConfig () {
    return new Promise((resolve, reject) => {
      ...
      if (exists(TARGET_FRONTEND_CONFIG_FILE)) {
        Message.info(`Storefront config file exist:'${TARGET_FRONTEND_CONFIG_FILE}', skipping...`)
        resolve()
        return
      }
      try {
  ...
  runDevEnvironment (answers) {
    return new Promise((resolve, reject) => {
      ...
      if (shell.exec(`nohup npm run dev >> ${Abstract.storefrontLogStream}`).code !== 0) {
...
let questions = [
  {
    ...
    default: !(process.env.QUESTIONS_DEFAULT_IS_REMOTE_BACKEND === 'false') /* dotenv parsed boolean as string so we need conversion */
  },
  {
    type: 'input',
    name: 'git_path',
  ...
  },
  {
    ...
    default: process.env.VUE_STOREFRONT_IMAGES_ENDPOINT || 'http://localhost:8080/img/',
    when: function (answers) {
      return answers.is_remote_backend === false
    }
  },
...
if (require.main.filename === __filename) {
  ...
  Message.info('which docker')
  shell.exec(`echo $(which docker)`)
  shell.exec(`echo $(which npm)`)
  process.exit(1)
